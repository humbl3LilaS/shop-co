            ...
            <PaginationContent>
                <PaginationItem>
                    <Button disabled={currentPage === 1} variant={"link"}>
                        <PaginationPrevious href={`${hrefBase}/?page=${currentPage - 1}`} />
                    </Button>
                </PaginationItem>
                {totalPages <= 4 &&
                    new Array(totalPages).fill(0).map((item, idx) => (
                        <PaginationItem
                            ...
                            className={cn(
                                "cursor-pointer",
                                currentPage === idx + 1 && "bg-black text-white rounded-lg",
                            )}
                        >
                            <PaginationLink href={`${hrefBase}/?page=${idx + 1}`}>
                                {idx + 1}
                            </PaginationLink>
                        </PaginationItem>
                    ))}
                ...
                {totalPages > 5 && (
                    <PaginationItem>
                        <PaginationLink
                            className={cn(
                                "cursor-pointer",
                                currentPage === totalPages && "bg-black text-white rounded-lg",
                            )}
                ...
                <PaginationItem>
                    <Button disabled={currentPage === totalPages} variant={"link"}>
                        <PaginationNext href={`${hrefBase}/?page=${currentPage + 1}`} />
                    </Button>
                </PaginationItem>
            </PaginationContent>
        </Pagination>
