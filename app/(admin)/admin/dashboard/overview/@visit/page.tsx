...
import VisitPercentage, {
    VisitPercentageVariant,
} from "@/feature/admin/overview/components/visit-percentage";
                ...
                <div className={"mt-8 flex flex-col justify-center gap-y-4"}>
                    {visitCount &&
                        Object.keys(visitCount)
                            .sort(
                                (a, b) =>
                                    visitCount[b as keyof IVisitsCount] -
                                    visitCount[a as keyof IVisitsCount],
                            )
                            .map((item) => (
                                <div key={item}>
                                    <p className={"mb-1 flex justify-between text-sm "}>
                                        ...
                                        <span className={"text-black/50"}>
                                            {calculateVisitPercentage(
                                                totalVisitCount,
                                                visitCount[item as keyof IVisitsCount],
                                            )}
                                            %
                                        </span>
                                    </p>
                                    <VisitPercentage
                                        value={calculateVisitPercentage(
                                            totalVisitCount,
                                            visitCount[item as keyof IVisitsCount],
                                        )}
