...
import CustomPagination from "@/components/client/custom-pagination";
    ...
    return (
        <Container>
            ...
            <div className={"lg:grid grid-cols-4 gap-x-5"}>
                <div
                    className={"hidden lg:block h-fit col-span-1 border border-black/20 rounded-xl"}
                >
                ...
                <div className={"lg:col-span-3"}>
                    <nav className={"mb-7 flex items-baseline gap-x-4 lg:justify-between"}>
                        ...
                        <p className={"text-black/60"}>
                            Showing {products.currentPage}-{products.totalPages} of &nbsp;
                            {products.totalProducts} Products
                        </p>
                        <FilterSheet defaultValues={formDefaultValues} />
                    </nav>
                    {products.data && products.data.length === 0 && (
                        <h1 className={"mt-10 text-2xl font-bold text-center lg:text-3xl"}>
                            No products
                        </h1>
                    )}
