...
const ImageGalleryUploader = ({ value, onChange }: ImageGalleryUploaderProps) => {
    ...
    const onReUpload = (idx: number): ChangeEventHandler<HTMLInputElement> => (evt) => {
        if (evt.target.files && evt.target.files[0]) {
            ...
            <div className={"flex gap-x-3"}>
                {
                    imageUrls && imageUrls.map((url, idx) =>
                        <div key={url} className={"relative w-[150px] aspect-square rounded-2xl z-10 group"}>
                            <Image src={url} alt={"preview image"} width={300} height={300} key={url}
                                   className={"w-[150px] aspect-square rounded-2xl"} />
                            <Label htmlFor={`image-gallery-edit-${idx}`}
                                   className={"absolute top-1/2 left-1/2 hidden w-3/4 px-2 py-3 bg-black text-white font-semibold rounded-lg -translate-x-1/2 -translate-y-1/2 group-hover:block"}>
                                Change Image
                            </Label>
                            <input
