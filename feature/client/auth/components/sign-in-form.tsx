...
import { SignInSchema, SignInSchemaType } from "@/validation/client-schema";
...
import { signIn } from "@/feature/client/auth/actions/sign-in";
...
const SignInForm = () => {
    ...
    const onSubmit: SubmitHandler<SignInSchemaType> = async (values) => {
        ...
        if (!auth.success) {
            toast({
                title: `Login Failed Invalid ${auth.error}`,
                variant: "destructive",
                duration: 500,
            });
        } else {
    ...
    return (
        <div
            className={
                "w-full max-w-screen-sm  px-4 py-6 rounded-2xl bg-white shadow-lg lg:max-w-screen-md md:p-10"
            }
        >
